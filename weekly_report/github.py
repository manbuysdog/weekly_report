...
class GitHub():
    ...
    def get_columns_and_issues(self, query):
        """
    ...
    def archive_done(url, done_cards):
        """
    ...
    def add_issue_to_project_column(url, column_id, issue_id):
        """
        ...
        response = requests.post(url, data=json.dumps(mutation),
                                headers={"Authorization": "token {}".format(get_github_token())})

        return response.json()


    def delete_card(url, card_id):
        """
    ...
    def get_issue_id_from_number(url, issue_number):
        """
